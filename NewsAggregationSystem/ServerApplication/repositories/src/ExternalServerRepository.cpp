...
std::vector<ExternalServer> ExternalServerRepository::getAllServers() {
    std::cout << "[ExternalServerRepository] getAllServers called" << std::endl;
    ...
    try {
        ...
        std::cout << "[ExternalServerRepository] getAllServers success" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "[ExternalServerRepository] getAllServers error: " << e.what() << std::endl;
    }
    return servers;
}

std::vector<ExternalServer> ExternalServerRepository::getAllServerDetails() {
    std::cout << "[ExternalServerRepository] getAllServerDetails called" << std::endl;
    std::vector<ExternalServer> servers;
    if (!db || !db->isConnected()) return servers;
    try {
        auto conn = db->getConnection();
        ...
        std::cout << "[ExternalServerRepository] getAllServerDetails success" << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "[ExternalServerRepository] getAllServerDetails error: " << e.what() << std::endl;
    }
    return servers;
}

bool ExternalServerRepository::updateApiKey(int id, const std::string& newKey) {
    std::cout << "[ExternalServerRepository] updateApiKey called" << std::endl;
    ...
    try {
        ...
        std::cout << "[ExternalServerRepository] updateApiKey success" << std::endl;
        return result;
    } catch (const std::exception& e) {
        std::cerr << "[ExternalServerRepository] updateApiKey error: " << e.what() << std::endl;
        return false;
    }
}
